...
class DataSetCatCon(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x_1 = np.array(self.X1[data_idx[0]])
        x_2 = np.array(self.X2[data_idx[0]])
        y = np.array(self.y[data_idx[0]])

        for _idx in np.array(data_idx)[1:]:
            # x_1.append(np.concatenate((self.cls[_idx], self.X1[_idx])))
            x_1 = np.append(x_1, self.X1[_idx], axis=0)
            x_2 = np.append(x_2, self.X2[_idx], axis=0)
            y = np.append(y, self.y[_idx], axis=0)
